...
class SwooleTrackerMiddleware implements MiddlewareInterface
{
	 ...
	 * @throws Throwable
	 */
	private function startAnalysis(Request $request): ?\SwooleTracker\Tick
	{
		$path    = $request->getUriPath();
	 ...
	 * @return void
	 */
	private function endNormalAnalysis(?\SwooleTracker\Tick $tick, int $responseCode): void
	{
		if (isset($tick)) {
	 ...
	 * @return void
	 */
	private function endExceptionAnalysis(?\SwooleTracker\Tick $tick, int $errno): void
	{
		if (isset($tick)) {
			$this->swoleTracker->endRpcAnalysis(
				$tick,
